...
while True:
    ...
    brightness = cv2.getTrackbarPos("Brightness", "Control") / 50.0
    contrast = cv2.getTrackbarPos("Contrast", "Control") / 50.0
    ...
    if results.pose_landmarks:
        for idx, lm in enumerate(results.pose_landmarks.landmark):
            ...
            if 0 <= px < w and 0 <= py < h:
                pose3d[idx] = (lm.x * w, lm.y * h, float(pred[py, px]))
    ...
    if all(i in pose3d for i in (15, 16, 23, 24)):
        if (
            abs(pose3d[15][0] - pose3d[23][0]) < 30
            and abs(pose3d[15][1] - pose3d[23][1]) < 30
            and abs(pose3d[16][0] - pose3d[24][0]) < 30
            and abs(pose3d[16][1] - pose3d[24][1]) < 30
        ):
            gestures.append("Hands on Hips")

    if all(i in pose3d for i in (12, 14, 16)):
        x_rs, y_rs = pose3d[12][0], pose3d[12][1]
        x_re, _ = pose3d[14][0], pose3d[14][1]
    ...
    if all(i in pose3d for i in (11, 13, 15)):
        ...
        x_le, _ = pose3d[13][0], pose3d[13][1]
    ...
    cv2.putText(
        frame,
        f"DL joints: {num_dl}  SIFT points: {num_sift}",
        (10, 25),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.6,
        (0, 255, 0),
    ...
    fps = 1.0 / max(time.time() - t_start, 1e-9)
    cv2.putText(
        combined,
        f"FPS: {fps:.1f}",
        (10, 50),
